...
public class Solution_프로세서연결 {
   ...
   public static int map[][], N, max, min, totalCnt;   //판, 판크기, 최대 코어수, 최소 전선길이, 처리할 코어수
   ...
   public static void main(String[] args) throws IOException {
      ...
      for (int TC = 1; TC <= T; TC++) {
         ...
         map = new int[N][N];
         list = new ArrayList<int[]>();   //처리해야 할, 가장자리가 아닌 코어들을 저장할 리스트
         max = 0;
         ...
         for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
               ...
               if((i==0 || j==0 || i==N-1 || j==N-1) && map[i][j] == 1) continue;   //가장자리에 있는 코어는 리스트 추가안함
               if(map[i][j]==1) {   //가장자리가 있지 않은 코어만 리스트에 추가
   ...
   private static void go(int index, int cCnt,int lCnt) {   //index: 처리할 코어의 index, cCnt: 직전까지 포함된 코어 수
	  ...
	  // 최대코어수가 임시해보다 작다면 진행이 의미 없음 
	   if(cCnt+totalCnt-index<max)return; //가지치기
 	   if(index == totalCnt) {
         if(max < cCnt) {
            max = cCnt;
            min = lCnt;
         } else if(max == cCnt) {   //최대 코어 갯수가 같은 경우
            if(min > lCnt) {
               min = lCnt;
            }
         }
   ...
   // 현재 코어의 위치에서 해당 방향으로 전선을 놓는게 가능한지 체크
   private static boolean isAvailable(int r, int c, int d) {
      int nr = r ,nc = c;
      while(true) {
         nr += dr[d];
         nc += dc[d];
         if(nr<0 || nr>=N || nc<0 || nc>=N) break;   //가장자리까지 전선을 놓을 수 있다
         if(map[nr][nc] >= 1) return false;   //1:코어, 2:전선
      }
      return true;
   }
   
   // 현재 코어의 위치에서 해당 방향으로 전선을 놓거나(s=2) 지우는(s=0) 셋팅
